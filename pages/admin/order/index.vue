<template>
  <div class="order adminContent">
    <AdminTitle :title="'配送集計表'" :create_btn_active="false"></AdminTitle>
    <SuccessFlash class="successMessage"></SuccessFlash>
    <div class="printing_btn_area flex _j-end">
    ...
    <p class="table_num" v-if="flag && orders.length || delimitedData.length">{{ currentPage }}ページ目を表示中 / 計{{ totalAmount }}ページ</p>
    ...
    <AdminOrderPrint 
      :usersOrderPrintData="usersOrderPrintData"
...
export default {
  ...
  async mounted() {
    ...
    this.ordersData = await this.$adminApi.getOrders(); 
  ...
  methods:{
    ...
    async usersOnPrint() {
      ...
      let delivery_location_ids = [];
      let resData = await this.$adminApi.searchUsers();
      let usersData = resData.users;

      for(let i = 0; i < this.orders.length; i++){
        for(let j = 0; j < this.orders[i].detail.length; j++){
          delivery_location_ids.push(this.orders[i].detail[j].delivery_location_id);
        }
      }

      let orders = [];
      // 注文があるユーザーの情報取得
      for(let i = 0; i < delivery_location_ids.length; i++){
        orders.push(await this.$adminApi.getOrderDetail(delivery_location_ids[i], this.deliveryDay));
      }
      ...
      for(let i = 0; i < orders.length; i++){
        ...
        // 納品場所と顧客の名前追加
        let delivery_location = this.delivery_locations.find((d) => d.id == orders[i].delivery_location_id);
        let client = this.clients.find((c) => c.id == delivery_location.client_id);
        if(delivery_location != undefined){
          orders[i].delivery_location_name = delivery_location.name;
          orders[i].client_name = client.company_name;
          for(let j = 0; j < orders[i].users.length; j++){
            // ユーザーのコード・名前・メールアドレス取得
            let user = usersData.find((u) => u.id == orders[i].users[j].user_id);
            orders[i].users[j].code = user.user_code;
            orders[i].users[j].name = user.last_name + user.first_name;
            orders[i].users[j].email = user.email;

            //商品合計数を計算
            for(let k = 0; k < orders[i].users[j].delivery_order.length; k++){
              let matched = product_ids.find((p) => p.product_id == orders[i].users[j].delivery_order[k].product_id);
              let index = product_ids.findIndex((o) => o.product_id == matched.product_id);
              product_ids[index].total += orders[i].users[j].delivery_order[k].count;
            }
          }
          orders[i].total_count = product_ids;
        }
        else{
          orders[i].delivery_location_name = "削除済です";
          orders[i].client_name = "";
        }
      }

      this.orderPrintData = [];
      this.usersOrderPrintData = orders;
      setTimeout(() => {
        window.print();
      }, 500);
    },
    ...
    // ユーザー無印刷
    // サーチApi使ってその日のデータ１ページに収めたものを印刷する
    async printOut() {
      this.isPrinting = true;
      this.orderPrintData = this.orders;
      this.usersOrderPrintData = [];
      setTimeout(() => {
        window.print();
      }, 500);
    ...
    async searchBtn(){
      ...
      if(searchUrl != undefined){
        this.isSearch = true;
        let resData = await this.$adminApi.searchOrders(searchUrl);
